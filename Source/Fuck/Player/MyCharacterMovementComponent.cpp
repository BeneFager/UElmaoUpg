...
bool UMyCharacterMovementComponent::AreRequiredWallRunKeysDown() const
{
	...
	// Make sure the spring key is down (the player may only wall run if he's holding sprint)
...
bool UMyCharacterMovementComponent::IsNextToWall(float vertical_tolerance)
{
	...
	if (vertical_tolerance > FLT_EPSILON)
	{
		...
		if (lineTrace(FVector(traceStart.X, traceStart.Y, traceStart.Z + vertical_tolerance / 2.0f),
		              FVector(traceEnd.X, traceEnd.Y, traceEnd.Z + vertical_tolerance / 2.0f)) == false &&
			lineTrace(FVector(traceStart.X, traceStart.Y, traceStart.Z - vertical_tolerance / 2.0f),
			          FVector(traceEnd.X, traceEnd.Y, traceEnd.Z - vertical_tolerance / 2.0f)) == false)
		{
			return false;
		}
...
{
	FVector crossVector;
	if (FVector2D::DotProduct(FVector2D(surface_normal), FVector2D(GetPawnOwner()->GetActorRightVector())) > 0.0f)
	{
                                               ...
                                               const FHitResult& Hit)
{
	...
	//MakeSure were falling. wall running can only begin if were in the aie
	if (IsFalling() == false)
		return;
...
void UMyCharacterMovementComponent::BeginPlay()
{
	...
	//We dont want simulated proxies detecting their own collision
	if (GetPawnOwner()->GetLocalRole() > ROLE_SimulatedProxy)
	{
		//Bind to the OnActorHit component so were notified when the owning actor hits something(like a wall)
                                                  ...
                                                  FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (GetPawnOwner()->IsLocallyControlled())
	{
		...
		{
			//Only set WatsToSprint ti true if the player is moving forward
		...
		//Update if the required wall run keys are being pressed
...
void UMyCharacterMovementComponent::OnMovementModeChanged(EMovementMode PreviousMovementMode, uint8 PreviousCustomMode)
{
	...
	if (PreviousMovementMode == MOVE_Custom)
	{
		...
		case ECustomMovementMode::CMOVE_WallRunning:
			{
				//Unconstrain the character from
...
void UMyCharacterMovementComponent::PhysWallRunning(float deltaTime, int32 Iterations)
{
	...
	//Make sure the required wall run keys are down
                               ...
                               class FNetworkPredictionData_Client_Character& ClientData)
{
	...
	UMyCharacterMovementComponent* charMov = static_cast<UMyCharacterMovementComponent*>(Character->
