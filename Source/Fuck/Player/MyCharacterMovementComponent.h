...
class FUCK_API UMyCharacterMovementComponent : public UCharacterMovementComponent
{
	GENERATED_BODY()
...
private:
	...
	//The amount of vertical room between the two linetraces when checking if the character is still on the wall
...
#pragma region Wall Running Functions
	...
	//Returns true if the movement mode is custom matches the provided custom movement mode
	bool IsCustomMovementMode(uint8 custom_movement_mode) const;

private:
	//Called when the owning actor hits something(to begin wall run)
	UFUNCTION()
...
#pragma region Private Variables
	...
	//direction the character is currently wall running in
